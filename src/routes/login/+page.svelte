...
<div class="flex flex-col items-center h-full w-full pt-32 sm:pt-40 md:pt-28">
	...
	<p class="text-center mt-1">
		หรือ <a href="/register" class="text-primary font-medium hover:cursor-pointer hover:underline yellow"
			>ลงทะเบียน</a
		> หากคุณยังไม่มีบัญชี
	</p>
	<form
	...
	>
		...
		<div class="w-full max-w-lg pt-2">
			<button type="submit" class="btn variant-filled-secondary btn-primary w-full" disabled={loading}>เข้าสู่ระบบ</button>
		</div>
		{#if form?.notVerified}
			<div class="alert alert-error shadow-lg w-full max-w-lg">
				<div>
					<svg
						...
						/></svg
					>
					<span>คุณต้องยืนยันอีเมลของคุณก่อนที่จะเข้าสู่ระบบได้</span>
				</div>
			</div>
		{/if}
		{#if form?.InvalidCred}
